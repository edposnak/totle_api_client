...
def print_savings_with_num_samples(savings_by_trade_size):
    ...
    for trade_size in sorted_trade_sizes(savings_by_trade_size):
        ...
        for exchange, pct_savings in savings_by_trade_size[trade_size].items():
            print(f"   {exchange}: {compute_mean(pct_savings):.2f}% ({n_samples} samples)")
...
def print_slippage_split_pct_csvs(slip_price_splits):
    ...
    for token, ts_agg_price_splits in slip_price_splits.items():
        ...
        for trade_size, agg_price_splits in ts_agg_price_splits.items():
            ...
            # get split_pct from JSON data, but fall back to CSV data if no JSON data available
            # TODO combine CSV and JSON data.
            split_pct = tokens_ts_pcts[token].get(trade_size) or compute_mean(splits)
...
def sorted_trade_sizes(*dicts):
    r = set(sum([ list(d.keys()) for d in dicts ], []))
    return map(str, sorted(map(float, r)))
...
per_token_non_splits_only_savings, _ = data_import.parse_csv_files(csv_files, only_non_splits=True)
...
print_non_split_savings_for_oneinch(per_token_non_splits_only_savings)
# print("\n\nSavings for each token by trade size (only splits)")
# print_per_token_savings_summary_tables(splits_only_savings, exchanges)
exit(0)
