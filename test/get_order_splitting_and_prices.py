...
QUOTE = 'ETH'
ALL_AGG_CLIENTS = [dexag_client, oneinch_client]
...
for base in TOKENS:
    for trade_size in TRADE_SIZES:
        ...
        # DEX.AG
        pq = dexag_client.get_quote(QUOTE, base, from_amount=trade_size, dex='all')
        if not pq:
            print(f"{DEX_AG} did not quote {QUOTE} to {base} at trade size={trade_size}")
        else:
            splits = exchange_utils.canonical_keys(pq['exchanges_parts'])
            dexs_used[DEX_AG] = splits
            dexs_with_pair |= splits.keys()
            dex_prices[DEX_AG] = pq['price']

            # print(f"{DEX_AG} had prices for {base}/{trade_size} ETH on {dexs_with_pair}")

        # 1-Inch
        pq = oneinch_client.get_quote(QUOTE, base, from_amount=trade_size)
        if not pq:
            print(f"{ONE_INCH} did not quote {QUOTE} to {base} at trade size={trade_size}")
        else:
            splits = exchange_utils.canonical_keys(pq['exchanges_parts'])
            dexs_used[ONE_INCH] = splits
            dexs_with_pair |= splits.keys()
            dex_prices[ONE_INCH] = pq['price']
