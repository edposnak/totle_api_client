...
def test_dex_name_map():
    dexs = oneinch_v3_client.exchanges()
    ...
    #     print(f"{can_name} => {oneinch_v3_client.DEX_NAME_MAP[can_name]}")
...
def test_get_quote(to_token, from_token='ETH', from_amount=None, to_amount=None, dex=None, verbose=True, debug=True):
    ...
    try:
        pq = oneinch_v3_client.get_quote(from_token, to_token, **kw_params)
        if not pq:
            ...
            print(pq)
    except (oneinch_v3_client.OneInchAPIException, ValueError) as e:
        print(e)
...
supp_tokens = oneinch_v3_client.supported_tokens()
