...
price_estimator = slippage_curves.PriceEstimator.construct(TOKEN, tok_ts_ex_pscs[TOKEN])
base_price = price_estimator.base_price

for ts, ex_pscs in tok_ts_ex_pscs[TOKEN].items():
    for ex, pscs in ex_pscs.items():
        mx_price = slippage_curves.first_price(pscs)
        ls_price = price_estimator.get_ls_price(ex, ts)
