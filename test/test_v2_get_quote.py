...
dex = '0xMesh'
# for base in [ 'DAI', 'ZRX']:
#     for trade_size in [0.1, 0.5, 1.0, 5.0]:
for base in ['DAI', 'ZRX']:
    for trade_size in [0.1, 5.0, 10.0, 15.0]:
        print(f"\n\n\ntoken={base} trade_size={trade_size}")
        pq = v2_client.get_quote(quote, base, from_amount=trade_size, debug=False)
        if not pq:
